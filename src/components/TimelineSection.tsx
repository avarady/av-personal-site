        ...
        return (
          <div className="mt-3">
              ...
              {item.imageSrc && 
                item.link ? (
                  <a href={item.link} target="_blank" rel="noreferrer">
                    <img className="timeline-img img-link" src={item.imageSrc} alt="logo" />
                  </a>
                ) : (
            ...
            <div className="timeline-list mt-2">
              <ul>
                {item.listItems && item.listItems.map((listItem: string, listIndex: number) => {
                  return (
                    <li className="mt-2" key={"item-" + index + "-listItem-" + listIndex}>
                      {listItem}
                    </li>)
                })}
              </ul>
